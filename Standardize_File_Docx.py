...
# Read: https://medium.com/@pemagrg/pre-processing-text-in-python-ad13ea544dae
...
def Standardize_File_Docx(file_docx_name):
    ...
    # Convert to unicode and lower words
    standard_content = unidecode.unidecode(
        standard_content.lower().replace('\n', ' '))

    # To expand the contractions
    standard_content = expand_contractions(standard_content, contractions_dict)

    # Remove speical characters
    standard_content = re.sub('[^A-Za-z0-9]+', ' ', standard_content)

    # Convert more than two space to one space
    standard_content = standard_content.replace('  ', ' ')

    # Remove special words!
    special_words = 'uh ah oh wow'
    for elem in special_words.split():
